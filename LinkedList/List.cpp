...
void List::Add(Node* newNode)
{
	...
	else //если список пуст, добавл€ем первый элемент
	{
		_head = newNode;
		_tail = newNode;
	}
...
{
	CheckIndex(index);
	if(index == 0) //если вставл€ем в начало списка
	{
		if(_head != NULL) //если список не пуст
		{
...
void List::CheckIndex(size_t& index)
{
	...
	{
		std::cout << "Ќеверный индекс -> " << index << '\n';
		exit(0);
	}
}
...
Node* List::RemoveByIndex(size_t index)
{
	...
	if(oldNode != NULL)
	{
		if(oldNode == _head) //если удал€ем головной элемент
		{
		...
		else if(oldNode == _tail) //если удал€ем последний элемент
		{
